...
<div class="max-w-2xl mx-auto p-6 bg-white rounded-2xl shadow-sm">
    <h2 class="text-xl font-bold text-gray-800 mb-6">{{ ($staffMode ?? false) ? 'កែសម្រួលគណនីបុគ្គលិក' : 'កែសម្រួលគណនីម្ចាស់ហាង ឬបុគ្គលិក' }}</h2>

    <form action="{{ route(($staffMode ?? false) ? 'staff.update' : 'users.update', $user->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">ឈ្មោះ</label>
            <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 @error('name') border-red-500 @else border-gray-300 @enderror" required>
            @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">អុីមែល (Email)</label>
            <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 @error('email') border-red-500 @else border-gray-300 @enderror" required>
        ...
        <div>
            ...
            <select name="role" class="w-full border-gray-300 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500" required>
                @if(!($staffMode ?? false))
                <option value="owner" {{ $user->role === 'owner' ? 'selected' : '' }}>Owner (ម្ចាស់ហាង)</option>
                @endif
        ...
        <div class="p-4 bg-yellow-50 text-yellow-800 rounded-lg text-xs mb-2">
            <i class="fa-solid fa-triangle-exclamation mr-1"></i><strong>ចំណាំ៖</strong> ទុកប្រអប់លេខសម្ងាត់ខាងក្រោមឱ្យនៅទទេ ប្រសិនបើអ្នកមិនចង់ផ្លាស់ប្តូរលេខសម្ងាត់របស់បុគ្គលិកម្នាក់នេះឡើយ។
        </div>

        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">លេខសម្ងាត់ថ្មី (Password)</label>
                <input type="password" name="password" class="w-full rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 @error('password') border-red-500 @else border-gray-300 @enderror">
